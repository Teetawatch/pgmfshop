...
<div class="container mx-auto px-4 py-8 max-w-4xl">
    ...
    <div class="flex items-center gap-2 text-sm text-gray-500 mb-6">
        ...
        <svg class="h-3 w-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="m9 18 6-6-6-6"/></svg>
        <a href="{{ route('account.orders.show', $order->id) }}" class="hover:text-gray-900 transition-colors">{{ $order->order_number }}</a>
        <svg class="h-3 w-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="m9 18 6-6-6-6"/></svg>
        <span class="text-gray-900 font-medium">ติดตามพัสดุ</span>
    </div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">คำสั่งซื้อ {{ $order->order_number }}</p>
        </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            @if($order->tracking_number)
                <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    <div class="bg-linear-to-r from-gray-700 to-gray-800 px-6 py-4">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center">
                                <svg class="h-5 w-5 text-white" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1M5 17a2 2 0 104 0m-4 0a2 2 0 114 0m6 0a2 2 0 104 0m-4 0a2 2 0 114 0"/></svg>
                            </div>
                            <div class="text-white">
                                <p class="text-sm font-medium opacity-90">เลขพัสดุ (ไปรษณีย์ไทย)</p>
                                <p class="text-xl font-bold font-mono tracking-wider">{{ $order->tracking_number }}</p>
                            </div>
                        </div>
                    </div>
                    <div class="px-6 py-4 flex flex-col sm:flex-row gap-3">
                        <a href="{{ $thaiPostUrl }}" target="_blank" rel="noopener noreferrer" class="flex-1 flex items-center justify-center gap-2 py-3 px-4 bg-orange-500 text-white text-sm font-semibold rounded-lg hover:bg-orange-600 transition-colors">
                        ...
                        <button onclick="navigator.clipboard.writeText('{{ $order->tracking_number }}').then(() => { this.querySelector('span').textContent = 'คัดลอกแล้ว!'; setTimeout(() => { this.querySelector('span').textContent = 'คัดลอกเลขพัสดุ'; }, 2000); })" class="flex items-center justify-center gap-2 py-3 px-4 border border-gray-200 text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors">
                            <svg class="h-4 w-4 text-gray-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
            ...
            @else
                <div class="bg-white rounded-xl border border-gray-200 p-8 text-center">
                    ...
                    @if($order->status === 'processing')
                        <div class="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 text-sm font-medium rounded-lg">
            ...
            <div class="bg-white rounded-xl border border-gray-200">
                <div class="px-6 py-4 border-b border-gray-100">
                    <h2 class="font-bold flex items-center gap-2">
                        <svg class="h-4 w-4 text-gray-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                        ประวัติสถานะ
                    </h2>
                </div>
                <div class="p-6">
                            ...
                            @foreach(array_reverse($history) as $idx => $entry)
                                @php
                                    ...
                                    $colorClass = match($entryStatus) {
                                        ...
                                        'cancelled', 'expired' => 'bg-red-500 text-white',
                                        default => $isFirst ? 'bg-gray-800 text-white' : 'bg-gray-200 text-gray-500',
                                    };
                                @endphp
                                <div class="flex gap-4 {{ !$loop->last ? 'pb-6' : '' }} relative">
                                    ...
                                    <div class="w-8 h-8 rounded-full {{ $colorClass }} flex items-center justify-center shrink-0 z-10">
                                        @if($isFirst)
                                            {!! $statusIcons[$entryStatus] ?? '<svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="4"/></svg>' !!}
                                        @else
                                    ...
                                    <div class="flex-1 min-w-0 pt-0.5">
                                        <div class="flex items-center gap-2 flex-wrap">
                                            ...
                                            @if($isFirst)
                                                <span class="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-green-100 text-green-700">ล่าสุด</span>
                                            @endif
                                        </div>
                                        @if($entryNote)
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200">
                <div class="px-5 py-4 border-b border-gray-100">
                    <h2 class="font-bold text-sm">สินค้าในคำสั่งซื้อ</h2>
                </div>
            ...
            <div class="bg-orange-50 rounded-xl border border-orange-200 p-5">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <a href="https://www.thailandpost.co.th" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1 text-xs text-orange-700 font-medium mt-2 hover:underline">
                            เว็บไซต์ไปรษณีย์ไทย
