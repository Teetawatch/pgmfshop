    ...
    @if(count($items) === 0)
        <div class="text-center py-16">
            ...
            <a href="{{ route('products') }}">
                <button class="px-6 py-3 bg-[hsl(var(--primary))] text-white rounded-md font-medium hover:opacity-90 transition-colors">เลือกซื้อสินค้า</button>
            </a>
        </div>
    @else
        ...
        <div class="grid lg:grid-cols-3 gap-8">
                ...
                @foreach($items as $item)
                    ...
                    <div class="bg-white rounded-lg border p-4">
                        <div class="flex gap-4">
                            <div class="w-24 h-24 rounded-md overflow-hidden bg-gray-100 shrink-0">
                                <img src="{{ $firstImage }}" alt="{{ $product->name }}" class="w-full h-full object-cover" />
                            </div>
                            <div class="flex-1 min-w-0">
                                <a href="{{ route('products.show', $product->slug) }}" class="font-medium hover:text-[hsl(var(--primary))] line-clamp-2">{{ $product->name }}</a>
                                ...
                                <div class="flex items-center justify-between mt-3">
                                    ...
                                    <div class="flex items-center gap-3">
                                        <span class="font-bold text-[hsl(var(--primary))]">฿{{ number_format($product->price * $qty, 0) }}</span>
            ...
            <div>
                <div class="bg-white rounded-lg border p-6 sticky top-24 space-y-4">
                    <h2 class="font-bold text-lg">สรุปคำสั่งซื้อ</h2>

                    <!-- Coupon -->
                    <div class="flex gap-2">
                        <input type="text" wire:model="couponCode" placeholder="รหัสคูปอง" class="flex-1 px-3 py-2 border border-gray-200 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-[hsl(var(--ring))]" />
                        <button wire:click="applyCoupon" class="px-4 py-2 border border-gray-200 rounded-md text-sm hover:bg-gray-50 transition-colors">ใช้</button>
                    </div>
                    ...
                    <a href="{{ route('checkout') }}" class="block">
                        <button class="w-full py-3 bg-[hsl(var(--primary))] text-white rounded-md font-medium hover:opacity-90 transition-colors">ดำเนินการสั่งซื้อ</button>
                    </a>
                </div>
            </div>
        </div>
