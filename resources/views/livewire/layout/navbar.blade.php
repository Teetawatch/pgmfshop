...
<header
    ...
    {{-- Expandable: Main Bar + Category Nav (collapses smoothly on scroll) --}}
    <div class="overflow-hidden transition-[max-height,opacity] duration-300 ease-in-out"
        :style="scrolled ? 'max-height:0;opacity:0;pointer-events:none' : 'max-height:200px;opacity:1;pointer-events:auto'">

        {{-- Main Bar --}}
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex h-16 items-center justify-between gap-6">
                <a href="/" class="flex items-center gap-2.5 shrink-0 group">
                    <img src="{{ vite_image('pgmf-logo.jpg') }}" alt="PGMF Shop" class="h-9 w-9 rounded-lg object-cover ring-1 ring-gray-200 group-hover:ring-gray-300 transition" />
                ...
                <div class="flex items-center gap-2">
                    @auth
                        <a href="{{ route('account.wishlist') }}" class="relative inline-flex items-center justify-center h-10 w-10 rounded-lg text-gray-500 hover:text-red-500 hover:bg-red-50 transition-colors" title="รายการโปรด">
                            ...
                            @if($wishlistCount > 0)<span class="absolute top-1 right-1 bg-red-500 text-white text-[10px] rounded-full h-4 min-w-4 px-1 flex items-center justify-center font-medium leading-none">{{ $wishlistCount }}</span>@endif
                        </a>
                        <a href="{{ route('cart') }}" class="relative inline-flex items-center justify-center h-10 w-10 rounded-lg text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition-colors">
                            ...
                            @if($cartCount > 0)<span class="absolute top-1 right-1 bg-gray-800 text-white text-[10px] rounded-full h-4 min-w-4 px-1 flex items-center justify-center font-medium leading-none">{{ $cartCount }}</span>@endif
                        </a>
                        <div class="hidden md:block relative" x-data="{ open: false }" @click.away="open = false">
                            <button @click="open = !open" class="flex items-center gap-2 h-10 pl-1 pr-3 rounded-lg hover:bg-gray-100 transition-colors">
                                ...
                                @else
                                    <div class="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center text-white text-xs font-semibold ring-2 ring-gray-100">{{ strtoupper(mb_substr($authUser->name, 0, 1)) }}</div>
                                @endif
        ...
        <div class="border-t border-b-2 border-orange-500 hidden md:block">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    {{-- Compact Bar (slides in when scrolled) --}}
    <div class="overflow-hidden transition-[max-height,opacity] duration-300 ease-in-out border-b-2 border-orange-500"
        :style="scrolled ? 'max-height:56px;opacity:1;' : 'max-height:0;opacity:0;pointer-events:none'">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-12 gap-2 md:gap-4">
                <div class="flex items-center gap-1 shrink-0">
                    ...
                    <div class="relative" x-data="{ catOpen: false }" @click.away="catOpen = false" @mouseenter="if(window.innerWidth >= 768) catOpen = true" @mouseleave="if(window.innerWidth >= 768) catOpen = false">
                        <button @click="catOpen = !catOpen" class="flex items-center gap-1.5 md:gap-2 text-sm font-medium px-2 md:px-3 py-2 text-gray-700 hover:text-gray-900 transition-colors">
                            <svg class="h-4 w-4 text-gray-600" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><line x1="4" x2="20" y1="6" y2="6"/><line x1="4" x2="20" y1="12" y2="12"/><line x1="4" x2="20" y1="18" y2="18"/></svg>
                ...
                <div class="flex items-center gap-0.5 md:gap-1 shrink-0">
                    ...
                    <a href="{{ route('cart') }}" class="relative inline-flex items-center justify-center h-9 w-9 rounded-lg text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-colors" title="ตะกร้าสินค้า">
                        <svg class="h-[18px] w-[18px]" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><circle cx="8" cy="21" r="1"/><circle cx="19" cy="21" r="1"/><path d="M2.05 2.05h2l2.66 12.42a2 2 0 002 1.58h9.78a2 2 0 001.95-1.57l1.65-7.43H5.12"/></svg>
                        @if($cartCount > 0)<span class="absolute top-0 right-0 bg-gray-800 text-white text-[10px] rounded-full h-4 min-w-4 px-1 flex items-center justify-center font-medium leading-none">{{ $cartCount }}</span>@endif
                    </a>
                </div>
            </div>
        </div>
