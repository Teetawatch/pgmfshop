...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
            <h2 class="text-xl font-bold text-gray-800">รายงานสินค้าขายดี</h2>
    ...
    <form method="GET" class="bg-white rounded-lg border p-4 mb-6">
        <div class="flex flex-wrap items-end gap-3">
            ...
            <div>
                <label class="text-sm font-medium text-gray-500 mb-1 block">จำนวน</label>
                <select name="limit" class="text-base border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-primary">
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-lg border p-4">
            <p class="text-sm text-gray-500 mb-1">สินค้าที่ขายได้</p>
            <p class="text-2xl font-bold text-gray-800">{{ number_format($summary['unique_products']) }} รายการ</p>
        </div>
        <div class="bg-white rounded-lg border p-4">
            <p class="text-sm text-gray-500 mb-1">จำนวนขายรวม</p>
            <p class="text-2xl font-bold text-primary">{{ number_format($summary['total_products_sold']) }} ชิ้น</p>
        </div>
        <div class="bg-white rounded-lg border p-4">
    ...
    <div class="bg-white rounded-lg border p-4 mb-6">
        <h3 class="text-base font-bold text-gray-700 mb-4">สินค้าขายดี Top 10</h3>
    ...
    <div class="bg-white rounded-lg border overflow-hidden">
        <div class="px-4 py-3 border-b">
            <h3 class="text-base font-bold text-gray-700">รายละเอียดสินค้าขายดี</h3>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-base">
                <thead>
                    <tr class="bg-gray-50 text-left text-sm text-gray-500">
                        <th class="px-4 py-3 font-medium text-base text-center w-10">#</th>
                        <th class="px-4 py-3 font-medium text-base">สินค้า</th>
                        <th class="px-4 py-3 font-medium text-base">หมวดหมู่</th>
                        <th class="px-4 py-3 font-medium text-base text-center">จำนวนขาย</th>
                        <th class="px-4 py-3 font-medium text-base text-right">ยอดขาย</th>
                        <th class="px-4 py-3 font-medium text-base text-center">ออเดอร์</th>
                        <th class="px-4 py-3 font-medium text-base text-right">ราคาเฉลี่ย</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($products as $idx => $p)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 text-center font-bold text-gray-400 text-base">{{ $idx + 1 }}</td>
                            <td class="px-4 py-3 text-base">
                                <div class="flex items-center gap-3">
                                    ...
                                    @endif
                                    <div>
                                        <p class="font-medium text-gray-800 line-clamp-1 text-base">{{ $p->product_name }}</p>
                            ...
                            <td class="px-4 py-3 text-gray-500 text-base">{{ $p->category_name }}</td>
                            <td class="px-4 py-3 text-center font-bold text-primary text-base">{{ number_format($p->total_sold) }}</td>
                            <td class="px-4 py-3 text-right font-medium text-base">฿{{ number_format($p->total_revenue, 0) }}</td>
                            <td class="px-4 py-3 text-center text-base">{{ number_format($p->order_count) }}</td>
                            <td class="px-4 py-3 text-right text-base">฿{{ number_format($p->avg_price, 0) }}</td>
                    ...
                    @empty
                        <tr><td colspan="8" class="px-4 py-8 text-center text-gray-400 text-base">ไม่มีข้อมูลในช่วงที่เลือก</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
