...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
            <h2 class="text-xl font-bold text-gray-800">รายงานสต็อกต่ำ</h2>
            <p class="text-base text-gray-500">สินค้าที่มีสต็อก ≤ {{ $threshold }} ชิ้น</p>
        </div>
    ...
    <form method="GET" class="bg-white rounded-lg border p-4 mb-6">
        <div class="flex flex-wrap items-end gap-3">
            <div>
                <label class="text-sm font-medium text-gray-500 mb-1 block">เกณฑ์สต็อกต่ำ</label>
                <select name="threshold" class="text-base border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-primary">
            ...
            <div>
                <label class="text-sm font-medium text-gray-500 mb-1 block">หมวดหมู่</label>
                <select name="category" class="text-base border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-primary">
            ...
            <div>
                <label class="text-sm font-medium text-gray-500 mb-1 block">เรียงตาม</label>
                <select name="sort" class="text-base border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-primary">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <div class="bg-white rounded-lg border p-4 border-l-4 border-l-red-500">
            <p class="text-sm text-gray-500 mb-1">สินค้าหมด</p>
        ...
        <div class="bg-white rounded-lg border p-4 border-l-4 border-l-orange-500">
            <p class="text-sm text-gray-500 mb-1">วิกฤต (1-5)</p>
            <p class="text-3xl font-bold text-orange-600">{{ $summary['critical'] }}</p>
        </div>
        <div class="bg-white rounded-lg border p-4 border-l-4 border-l-amber-500">
            <p class="text-sm text-gray-500 mb-1">สต็อกต่ำ (6-{{ $threshold }})</p>
            <p class="text-3xl font-bold text-amber-600">{{ $summary['low'] }}</p>
        </div>
        <div class="bg-white rounded-lg border p-4 border-l-4 border-l-gray-400">
            <p class="text-sm text-gray-500 mb-1">รวมทั้งหมด</p>
        ...
        <div class="bg-white rounded-lg border p-4 border-l-4 border-l-primary">
            <p class="text-sm text-gray-500 mb-1">มูลค่าคงเหลือ</p>
            <p class="text-2xl font-bold text-primary">฿{{ number_format($summary['total_value'], 0) }}</p>
        </div>
    </div>
    ...
    <div class="grid lg:grid-cols-4 gap-6 mb-6">
        ...
        <div class="bg-white rounded-lg border p-4">
            <h3 class="text-base font-bold text-gray-700 mb-4">แยกตามหมวดหมู่</h3>
        ...
        <div class="lg:col-span-3 bg-white rounded-lg border overflow-hidden">
            <div class="px-4 py-3 border-b">
                <h3 class="text-base font-bold text-gray-700">รายการสินค้า ({{ $products->count() }} รายการ)</h3>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-base">
                    <thead>
                        <tr class="bg-gray-50 text-left text-sm text-gray-500">
                            <th class="px-4 py-3 font-medium text-base">สินค้า</th>
                            <th class="px-4 py-3 font-medium text-base">หมวดหมู่</th>
                            <th class="px-4 py-3 font-medium text-base text-center">สต็อก</th>
                            <th class="px-4 py-3 font-medium text-base text-center">ขายแล้ว</th>
                            <th class="px-4 py-3 font-medium text-base text-right">ราคา</th>
                            <th class="px-4 py-3 font-medium text-base text-right">มูลค่าคงเหลือ</th>
                            <th class="px-4 py-3 font-medium text-base text-center">สถานะ</th>
                            <th class="px-4 py-3 font-medium text-base text-center">จัดการ</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50 {{ $p->stock === 0 ? 'bg-red-50/50' : '' }}">
                                <td class="px-4 py-3 text-base">
                                    <div class="flex items-center gap-3">
                                        ...
                                        @endif
                                        <p class="font-medium text-gray-800 line-clamp-1 text-base">{{ $p->name }}</p>
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-gray-500 text-base">{{ $p->category->name ?? '-' }}</td>
                                ...
                                <td class="px-4 py-3 text-center text-gray-500 text-base">{{ number_format($p->sold) }}</td>
                                <td class="px-4 py-3 text-right text-base">฿{{ number_format($p->price, 0) }}</td>
                                <td class="px-4 py-3 text-right font-medium text-base">฿{{ number_format($p->stock * $p->price, 0) }}</td>
                                <td class="px-4 py-3 text-center text-base">
                                    @if($p->stock === 0)
                        ...
                        @empty
                            <tr><td colspan="8" class="px-4 py-8 text-center text-gray-400 text-base">ไม่มีสินค้าที่ต่ำกว่าเกณฑ์</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
